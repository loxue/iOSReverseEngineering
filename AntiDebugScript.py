...
反调试脚本，过了反调试之后记得：
aadebug -d
否则，程序运行会很慢。如果有定时器定时检测，建议编写Tweak
'''
...
class AMAntiAntiDebug(fb,FBCommand):
    ...
    def antiPtrace(self):
        ptraace = lldb.debugger.GetSelectedTarget().BreakpointCreateByName("ptrace")
        if isMac():
...
def ptrace_callback(frame, bp_loc, internal_dict):
    ...
    lldb.debugger.HandleCommand('conginue')

def syscall_callback(frame, bp_loc, internal_dict):
    print "find syscall"
    lldb.debugger.GetSelectedTarget().GetProgress().SetSelectedThread(frame.GetThread())
    if isMac():
    ...
    lldb.debugger.HandleCommand('contigue')
